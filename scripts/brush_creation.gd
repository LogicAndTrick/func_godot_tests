...
static func intersection_single_double(generator : FuncGodotGeometryGenerator, brush : FuncGodotData.BrushData):
	return _intersection(generator, brush, generate_face_vertices_hyperplane_double)

## plane intersection using float64 intersections & float32 hyperplanes
static func intersection_double_single(generator : FuncGodotGeometryGenerator, brush : FuncGodotData.BrushData):
	return _intersection64(generator, brush, generate_face_vertices_hyperplane_single)

## plane intersection using float64 intersections & float64 hyperplanes
static func intersection_double_double(generator : FuncGodotGeometryGenerator, brush : FuncGodotData.BrushData):
	return _intersection64(generator, brush, generate_face_vertices_hyperplane_double)
...
static func _intersection64(generator : FuncGodotGeometryGenerator, brush : FuncGodotData.BrushData, hyperplane_callback : Callable):
	# as above, except we do the intersection with float64s
	var num := brush.faces.size()
	var cloud : Array[Array] = []
